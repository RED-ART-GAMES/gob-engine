...
#include "gob/levels.h"
 ...
 // To display hotspots, for Gobliiins 1, 2 and 3
#include "common/graphics.h"

namespace Gob {
	ManagedCheats::ManagedCheats() {
		_cheatsToDisplay = std::map<std::string, std::vector<std::tuple<Kind, uint16, int16, int16>>>();
		_customCheats = std::map<std::string, std::vector<std::tuple<Common::String, Common::Point, Common::Point>>>();
	}

	ManagedCheats::~ManagedCheats() {
		_cheatsToDisplay.clear();
		_customCheats.clear();
	}

	void ManagedCheats::clear() {
		_cheatsToDisplay.clear();
		_customCheats.clear();
	}

	void ManagedCheats::allowToDisplay(const std::string& levelName, const ManagedCheats::Kind kind, const uint16 id, const int16 x_offset, const int16 y_offset) {
		std::string uppercaseLevelName = levelName;
		std::transform(uppercaseLevelName.begin(), uppercaseLevelName.end(), uppercaseLevelName.begin(), ::toupper);
		auto it = _cheatsToDisplay.find(uppercaseLevelName);
		if (_cheatsToDisplay.end() != it) {
			(it->second).emplace_back(std::make_tuple(kind, id, x_offset, y_offset));
			return;
		}
		std::vector initVector = std::vector<std::tuple<Gob::ManagedCheats::Kind, uint16, int16, int16>>();
		initVector.emplace_back(std::make_tuple(kind, id, x_offset, y_offset));
		_cheatsToDisplay.insert(std::pair(uppercaseLevelName, initVector));
	}

	bool ManagedCheats::canDisplay(const std::string& levelName, const ManagedCheats::Kind kind, const uint16 id) {
#if defined(_DEBUG_HOTSPOTS)
		// Allow to display anything in this mode
		//return true;
#endif
		std::string uppercaseLevelName = levelName;
		std::transform(uppercaseLevelName.begin(), uppercaseLevelName.end(), uppercaseLevelName.begin(), ::toupper);
		auto it = _cheatsToDisplay.find(uppercaseLevelName);
		if (_cheatsToDisplay.end() == it) return false;
		for (const auto& [cheatKind, cheatId, cheatX_offset, cheatY_offset] : it->second) {
			if (cheatKind == kind && cheatId == id) return true;
		}
		return false;
	}

	uint16 ManagedCheats::getObjectOffsetX(const std::string& levelName, const ManagedCheats::Kind kind, const uint16 id) {
		std::string uppercaseLevelName = levelName;
		std::transform(uppercaseLevelName.begin(), uppercaseLevelName.end(), uppercaseLevelName.begin(), ::toupper);
		auto it = _cheatsToDisplay.find(uppercaseLevelName);
		if (_cheatsToDisplay.end() == it) return false;
		for (const auto& [cheatKind, cheatId, cheatX_offset, cheatY_offset] : it->second) {
			if (cheatKind == kind && cheatId == id) return cheatX_offset;
		}
		return 0;
	}

	uint16 ManagedCheats::getObjectOffsetY(const std::string& levelName, const ManagedCheats::Kind kind, const uint16 id) {
		std::string uppercaseLevelName = levelName;
		std::transform(uppercaseLevelName.begin(), uppercaseLevelName.end(), uppercaseLevelName.begin(), ::toupper);
		auto it = _cheatsToDisplay.find(uppercaseLevelName);
		if (_cheatsToDisplay.end() == it) return false;
		for (const auto& [cheatKind, cheatId, cheatX_offset, cheatY_offset] : it->second) {
			if (cheatKind == kind && cheatId == id) return cheatY_offset;
		}
		return 0;
	}

	void ManagedCheats::addCustomCheat(const std::string& levelName, const Common::String& id, const Common::Point& topLeft, const Common::Point& bottomRight) {
		std::string uppercaseLevelName = levelName;
		std::transform(uppercaseLevelName.begin(), uppercaseLevelName.end(), uppercaseLevelName.begin(), ::toupper);
		auto it = _customCheats.find(uppercaseLevelName);
		if (_customCheats.end() != it) {
			(it->second).emplace_back(std::make_tuple(id, topLeft, bottomRight));
			return;
		}
		std::vector initVector = std::vector<std::tuple<Common::String, Common::Point, Common::Point>>();
		initVector.emplace_back(std::make_tuple(id, topLeft, bottomRight));
		_customCheats.insert(std::pair(uppercaseLevelName, initVector));
	}

	bool ManagedCheats::existsCustomCheatsFor(const std::string& levelName) {
		std::string uppercaseLevelName = levelName;
		std::transform(uppercaseLevelName.begin(), uppercaseLevelName.end(), uppercaseLevelName.begin(), ::toupper);
		auto it = _customCheats.find(uppercaseLevelName);
		return !(_customCheats.end() == it);
	}

	std::vector<std::tuple<Common::String, Common::Point, Common::Point>>& ManagedCheats::getCustomCheats(const std::string& levelName) {
		std::string uppercaseLevelName = levelName;
		std::transform(uppercaseLevelName.begin(), uppercaseLevelName.end(), uppercaseLevelName.begin(), ::toupper);
		auto it = _customCheats.find(uppercaseLevelName);
		return it->second;
	...
	uint16 Hotspots::add(const Hotspot& hotspot) {
		for (int i = 0; i < kHotspotCount; i++) {
			...
			debug(SoftLevel::kHotSpots, "Adding hotspot %03d: Coord:%3d+%3d+%3d+%3d - id:%04X, key:%04X, flag:%04X - fcts:%5d, %5d, %5d",
	...
	void Hotspots::remove(uint16 id) {
		for (int i = 0; i < kHotspotCount; i++) {
			if (_hotspots[i].id == id) {
				debug(SoftLevel::kHotSpots, "Removing hotspot %d: %X", i, id);
				_hotspots[i].clear();
			}
		}
	}
	...
	void Hotspots::removeState(uint8 state) {
		for (int i = 0; i < kHotspotCount; i++) {
			...
			if (spot.getState() == state) {
				debug(SoftLevel::kHotSpots, "Removing hotspot %d: %X (by state %X)", i, spot.id, state);
				spot.clear();
			}
		}
	}
	...
	void Hotspots::recalculate(bool force) {
		GraphicsManager* manager = g_system->getGraphicsManager();
		if (NULL == manager) return; // Should not happen

		manager->clearHotspotCheatsList();
		// Send the current scroll offsets
		{
			Common::Point convertedScrollOffsets = manager->convertVirtualToWindow(_vm->_video->_scrollOffsetX, _vm->_video->_scrollOffsetY);
			manager->setScrollPosition(convertedScrollOffsets.x, convertedScrollOffsets.y);
		}

		const std::string levelName = std::string(_vm->_game->_curTotFile.c_str());
		...
		for (int i = 0; (i < kHotspotCount) && !_hotspots[i].isEnd(); i++) {
			...
			if (spot.funcPos == 0) {
				...
				if (_vm->_game->_managedCheats->canDisplay(levelName, Gob::ManagedCheats::Kind::kHotspot, spot.id)) {
					// However push it in the current list of hotspots
					int16 offset_x = _vm->_game->_managedCheats->getObjectOffsetX(levelName, Gob::ManagedCheats::Kind::kHotspot, spot.id);
					int16 offset_y = _vm->_game->_managedCheats->getObjectOffsetY(levelName, Gob::ManagedCheats::Kind::kHotspot, spot.id);
					Common::Point topLeft(spot.left, spot.top);
					Common::Point bottomRight(spot.right, spot.bottom);
					manager->addHotspotCheat(spot.id, _vm->_game->_curTotFile, topLeft, bottomRight, offset_x, offset_y);
				}
				continue;
			}
		...
		if (!_vm->_game->_managedCheats->existsCustomCheatsFor(levelName)) return;
		auto& _customCheatsCache = _vm->_game->_managedCheats->getCustomCheats(levelName);
		// In contrary of official objects / hotspots cheats, there is no need to convert the coordinates from virtual (game) space to window space, 
		// as this has already been computed in the ManagedCheats object
		//
		// TOCHECK : isn't that better to keep a reference to the internal vector, instead of copying the values?
		// Safer to copy, and copying once per level, so this should be fine in terms of performance reasons...
		//
		// NOTE(Florian): if we want to copy hotspots once per level load, this is not the right place to do it, because this is called every frame.
		for (const auto& [id, topLeft, bottomRight]: _customCheatsCache) manager->addCustomCheat(Common::String(id.c_str()), topLeft, bottomRight);
	}

	void Hotspots::push(uint8 all, bool force) {
		debug(SoftLevel::kHotSpots, "Pushing hotspots (%d, %d)", all, force);
	...
	void Hotspots::pop() {
		debug(SoftLevel::kHotSpots, "Popping hotspots");
	...
	void Hotspots::enter(uint16 index) {
		debug(SoftLevel::kHotSpots, "Entering hotspot %d", index);
		...
		if (spot.funcEnter != 0) call(spot.funcEnter);
	}

	void Hotspots::leave(uint16 index) {
		debug(SoftLevel::kHotSpots, "Leaving hotspot %d", index);
		...
		if (spot.funcLeave != 0) call(spot.funcLeave);
	}
	...
	uint16 Hotspots::checkMouse(Type type, uint16& id, uint16& index) const {
		...
		if (type == kTypeMove) {
			...
			for (int i = 0; (i < kHotspotCount) && !_hotspots[i].isEnd(); i++) {
				...
				if ((_vm->getGameType() == kGameTypeGob2) && _vm->isCurrentTot("gob18.tot") && _hotspots[i].id == 32786 && _hotspots[i].left == 0 && _hotspots[i].right == 1)
					//RAG: hack to avoid a softlock in Gob2 where you can enter the "Giant" room from the "Kael" room before the Prince Buffoon is abducted by a bird
					//the "Giant" room hotspot is clickable too soon, as it is already clickable from strange coordinates in the very left of the room.
					//So we ignore this hotspot until it is placed at the proper coordinates
					continue;
			...
			for (int i = 0; (i < kHotspotCount) && !_hotspots[i].isEnd(); i++) {
				...
				if ((_vm->getGameType() == kGameTypeGob2) && _vm->isCurrentTot("gob18.tot") && _hotspots[i].id == 32786 && _hotspots[i].left == 0 && _hotspots[i].right == 1)
					//RAG SC51561: hack to avoid a softlock in Gob2 where you can enter the "Giant" room from the "Kael" room before the Prince Buffoon is abducted by a bird
					//the "Giant" room hotspot is clickable too soon, as it is already clickable from strange coordinates in the very left of the room.
					//So we ignore this hotspot until it is placed at the proper coordinates
					continue;
	...
	uint16 Hotspots::check(uint8 handleMouse, int16 delay, uint16& id, uint16& index) {
		...
		while (key == 0) {
			...
			// Break while saving
			if (_vm->_game->isPlayerStatePaused()) { break; }
		}
	...
	uint16 Hotspots::updateInput(uint16 xPos, uint16 yPos, uint16 width, uint16 height,
		...
		while (1) {
			...
			while (1) {
				...
				// Skip the text field in Gob2 or Gob3, once the menu is displayed
				// The text **should** be printed otherwise scroll issues will happen for Gob3
				// The check, at this position, fixes issue #22991
				if (_vm->isCurrentTot("menu.tot") && (_vm->getGameType() == Gob::GameType::kGameTypeGob2 || _vm->getGameType() == Gob::GameType::kGameTypeGob3)) {
					key = kKeyReturn; // Simulate 'hit Return key'
					break;
				}
	...
	void Hotspots::evaluateNew(uint16 i, uint16* ids, InputDesc* inputs,
		...
		if ((windowNum != 0) && (type != 0) && (type != 2))
			debug(SoftLevel::kHotSpots, "evaluateNew - type %d, win %d", type, windowNum);
	...
	void Hotspots::evaluate() {
		...
		while ((id == 0) && !_vm->_inter->_terminate && !_vm->shouldQuit()) {
			uint16 key = 0;
			if (hasInput) {
				if (_vm->getGameType() == Gob::GameType::kGameTypeGob1 && _vm->_game->shouldFirePasswordInputs) {
					// Get the password for the current level
					std::optional<Common::String> foundPassword = Gob::Levels::Gob1::getPasswordFromLevel(_vm->_game->_curTotFile);
					if (foundPassword.has_value()) {
						Common::String password = foundPassword.value();
						for (const char c : password) { // Type each character
							{
								Common::Event event;
								event.type = Common::EventType::EVENT_KEYDOWN;
								event.kbd.keycode = (Common::KeyCode)(c);
								event.kbd.ascii = c;
								g_system->getEventManager()->notifyEvent(event);
							}
							{
								Common::Event event;
								event.type = Common::EventType::EVENT_KEYUP;
								event.kbd.keycode = (Common::KeyCode)(c);
								event.kbd.ascii = c;
								g_system->getEventManager()->notifyEvent(event);
							}
						}
						// Return key to fire the event
						{
							Common::Event event;
							event.type = Common::EventType::EVENT_KEYDOWN;
					...
					_vm->_game->shouldFirePasswordInputs = false;
				}
				else {
					// Input
					uint16 curInput = 0;
				...
				_vm->_gob1_isShowingGameRetryDialog = false;
			}
			...
			if ((!_vm->_game->isPlayerStatePaused() && (id == 0)) || (_hotspots[index].funcLeave != 0))
