...
namespace Gob {
	...
	void Mult::playMult(int16 startFrame, int16 endFrame, char checkEscape,
		...
		// WORKAROUND
		// This skips automatically the intro video in Gob2 Floppy USA
		// Only one frame is being displayed (because of the do...while loop), but this should not be
		// an issue as we check for the loadFirstSlotAutomatically flag
		while (!stop && !stopNoClear && !_vm->shouldQuit() && _vm->_game->loadFirstSlotAutomatically == Gob::Game::LoadAutoState::NOT_PLANNED);
