...
namespace Gob {
	...
	void Mult_v2::loadMult(int16 resId) {
		...
		debug(SoftLevel::kInterpreter, "Loading mult %d", index);
		...
		debug(SoftLevel::kInterpreter, "statics: %u, anims: %u, imds: %u",
	...
	void Mult_v2::setMultData(uint16 multIndex) {
		...
		debug(SoftLevel::kInterpreter, "Switching to mult %d", multIndex);
	...
	void Mult_v2::multSub(uint16 multIndex) {
		...
		debug(SoftLevel::kInterpreter, "Sub mult %d", multIndex);
		_multData = _multDatas[multIndex];
	...
	void Mult_v2::playImd(const char* imdFile, Mult::Mult_ImdKey& key, int16 dir,
		...
		debug(SoftLevel::kVideoPlayer, "Playing mult video \"%s\" @ %d+%d, frame %d, "
