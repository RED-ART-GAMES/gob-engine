...
namespace Gob {
	...
	/// <summary>
	/// Return the save data filename for the current game
	/// The save data filename is computed based on:
	/// * the game type (Gob1, Gob2, Gob3, ...)
	/// * the associated feature of the current version (floppy, cd, ...)
	/// If any of those information is wrong then it crashes.
	/// </summary>
	/// <returns>The save data filename as a string. Does not contains the mount point!</returns>
	...
	public:
		...
		/** For consoles. */
		virtual bool commitChanges(const char* filename);
