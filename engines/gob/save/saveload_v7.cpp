...
namespace Gob {
	...
	SaveLoad_v7::DrawingOnFloppyDiskHandler::DrawingOnFloppyDiskHandler(GobEngine* vm,
		...
		bool isThumbnail,
		uint32 chunkSize)
		: TempSpriteHandler(vm), _writer(writer), _reader(reader), _isThumbnail(isThumbnail), _chunkSize(chunkSize) {
	}
	...
	SaveLoad_v7::SaveLoad_v7(GobEngine* vm, const char* targetName) :
		...
		SaveReader* reader = new SaveReader(_vm, nbrOfParts, 0, drawingOnFloppyDiskName);
