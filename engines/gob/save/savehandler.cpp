...
#define DEFAULT_CHAPTER_STR "Chapter"
#define DEFAULT_FIRST_CHAPTER_STR "Latest Sav"
	// WARNING: do not put a string with more than 8 characters!
	static const std::map<Common::Language, const char*> CHAPTERS_FILENAME_REPLACEMENT = {
		{ Common::Language::EN_ANY, DEFAULT_CHAPTER_STR },
		{ Common::Language::EN_GRB, DEFAULT_CHAPTER_STR },
		{ Common::Language::EN_USA, DEFAULT_CHAPTER_STR },
		{ Common::Language::FR_FRA, "Chapitre" },
		{ Common::Language::DE_DEU, "Kapitel" },
		{ Common::Language::ES_ESP, new char[9] { 'C', 'a', 'p', char(161), 't', 'u', 'l', 'o', '\0'}},
		{ Common::Language::IT_ITA, "Capitolo" }
	};
	static const std::map<Common::Language, const char*> FIRST_SLOT_FILENAME_REPLACEMENT = {
		{ Common::Language::EN_ANY, DEFAULT_FIRST_CHAPTER_STR },
		{ Common::Language::EN_GRB, DEFAULT_FIRST_CHAPTER_STR },
		{ Common::Language::EN_USA, DEFAULT_FIRST_CHAPTER_STR },
		{ Common::Language::FR_FRA, "Dern.Sauv." },
		{ Common::Language::DE_DEU, "Aktuelle" },
		{ Common::Language::ES_ESP, "Actual" },
		{ Common::Language::IT_ITA, "Attuale" }
	};
	static const char* RAG_TAG_DESC = (char*)"RRAGG";

	// See issue #23020.
	const char* GetSaveSlotLanguageDirectory(const GobEngine& gobEngine) {
		switch (gobEngine.getGameType()) {
			case Gob::GameType::kGameTypeGob2: [[fallthrough]];
			case Gob::GameType::kGameTypeGob3: {
				switch (gobEngine._language) {
					case Common::Language::EN_ANY: [[fallthrough]];
					case Common::Language::EN_GRB: return "EN_GRB";

					case Common::Language::EN_USA: return "EN_USA";
					case Common::Language::FR_FRA: return "FR";
					case Common::Language::DE_DEU: return "DE";
					case Common::Language::ES_ESP: return "SP";
					case Common::Language::IT_ITA: return "IT";
				}

				break;
			}

			// Not sure where that would apply. Maybe Woodruff?
			default: {
				switch (gobEngine._language) {
					case Common::Language::EN_ANY: [[fallthrough]];
					case Common::Language::EN_GRB: [[fallthrough]];
					case Common::Language::EN_USA: return "EN";
		...
		error("Can't deduce the right save folder from the current game/language combination!");

		return "";
	}

	bool LoadSavesFromBothRamAndSaves(const GobEngine& gobEngine) {
		return gobEngine.getGameType() == kGameTypeGob2 || gobEngine.getGameType() == kGameTypeGob3;
	}
	...
	uint32 SlotFileIndexed::getSlotMax() const {
		...
		for (int i = (_slotCount - 1); i >= 0; i--) {
			...
			in = saveMan->openFileOrSaveForLoading(
				LoadSavesFromBothRamAndSaves(*_vm),
				slotFile,
				GetSaveSlotLanguageDirectory(*_vm));
	...
	void replaceSlotFilename(char* description, const Common::Language language, const uint32 slotId) {
		if (slotId == 0) {
			// The first slot is special, as it is not considered as a "chapter"
			memset(description, 0x0, 40); // TODO : find the proper define for that length
			auto replacementString = FIRST_SLOT_FILENAME_REPLACEMENT.find(language);
			if (replacementString == FIRST_SLOT_FILENAME_REPLACEMENT.end())
				strncpy(description, DEFAULT_FIRST_CHAPTER_STR, strlen(DEFAULT_FIRST_CHAPTER_STR));
			else
				strncpy(description, replacementString->second, strlen(replacementString->second));
			return;
		}

		// Look for the pattern to replace
		std::string prepareToReplace = std::string(description);
		int index = -1;
		index = prepareToReplace.find(RAG_TAG_DESC);

		if (index != std::string::npos) {
			// Found the pattern
			auto replacementString = CHAPTERS_FILENAME_REPLACEMENT.find(language);
			if (replacementString == CHAPTERS_FILENAME_REPLACEMENT.end()) prepareToReplace.replace(index, strlen(RAG_TAG_DESC), DEFAULT_CHAPTER_STR);
			else prepareToReplace.replace(index, strlen(RAG_TAG_DESC), replacementString->second);
			strncpy(description, prepareToReplace.c_str(), prepareToReplace.length());
			description[prepareToReplace.length() + 1] = '\0';
		}
	}
	...
	void SlotFileIndexed::buildIndex(byte* buffer, SavePartInfo& info,
		...
		// Let the user load all the save states until the current level (or chapter) reached, including the current one
		// (as the player can softlock himself in certain chapters)
		for (uint32 i = 0; i < static_cast<int>(_vm->_game->_highestReachedLevel) + 1; i++, buffer += descLength) {
			Common::String slotFile = build(i);
			if (!slotFile.empty()) {
				char* desc = nullptr;
				if (converter && (desc = converter->getDescription(slotFile))) {
					// Old style save
					replaceSlotFilename(desc, _vm->_language, i);
					memcpy(buffer, desc, descLength);
				}
				else if (SaveReader::getInfo(*_vm, slotFile, info)) {
					// New style save
					char descToReplace[40];
					memset(descToReplace, 0x0, 40);
					const char* currentDesc = info.getDesc();
					strcpy(descToReplace, currentDesc);
					replaceSlotFilename(descToReplace, _vm->_language, i);
					memcpy(buffer, descToReplace, descLength);
				}
				else {
			...
			else memset(buffer, 0, descLength); // No valid slot, fill with 0
		}

		if (setLongest) {
			uint32 slot0Len;
			for (slot0Len = strlen((const char*)bufferStart); slot0Len < longest; slot0Len++) bufferStart[slot0Len] = ' ';
	...
	Common::InSaveFile* SlotFileIndexed::openRead(int slot) const {
		...
		return g_system
			->getSavefileManager()
			->openFileOrSaveForLoading(LoadSavesFromBothRamAndSaves(*_vm), name, GetSaveSlotLanguageDirectory(*_vm));
	}
	...
	Common::OutSaveFile* SlotFileIndexed::openWrite(int slot) const {
		...
		if (name.empty()) return nullptr;
		Common::SaveFileManager* saveMan = g_system->getSavefileManager();
		Common::OutSaveFile* result = saveMan->openSaveForSaving(name);
		return result;
	}
	...
	bool TempSpriteHandler::createFromSprite(int16 dataVar, int32 size, int32 offset) {
		return (bool)createSprite(dataVar, size, offset);
	}
	...
	int32 NotesHandler::getSize() {
		...
		SaveReader reader(_vm, 1, 0, fileName);
		SaveHeader header;
	...
	bool NotesHandler::load(int16 dataVar, int32 size, int32 offset) {
		...
		if (converter.isOldSave()) {
			...
			reader = new SaveReader(_vm, 1, 0, converter);

		}
		else
			// New save, load directly
			reader = new SaveReader(_vm, 1, 0, fileName);
