...
namespace Gob {
	...
	SavePartVars::SavePartVars(GobEngine* vm, uint32 size) : SavePart(), _vm(vm), _size(size) {
		_header.setType(kID);
		_header.setVersion(kVersion);
		_header.setSize(size);
	...
	SaveReader::SaveReader(const GobEngine* gobEngine, uint32 partCount, uint32 slot, const Common::String& fileName)
		: SaveContainer(partCount, slot)
		, _fileName(fileName)
		, _gobEngine(gobEngine) {
		_stream = nullptr;
		_loaded = false;
	}

	SaveReader::SaveReader(const GobEngine* gobEngine, uint32 partCount, uint32 slot, Common::SeekableReadStream& stream)
		: SaveContainer(partCount, slot)
		, _gobEngine(gobEngine) {
		_stream = &stream;
		_loaded = false;
	...
	Common::InSaveFile* SaveReader::openSave(const GobEngine& gobEngine, const Common::String& fileName) {
		if (fileName.empty()) {
			return nullptr;
		}
		...
		return g_system
		->getSavefileManager()
		->openFileOrSaveForLoading(
			LoadSavesFromBothRamAndSaves(gobEngine),
			fileName,
			GetSaveSlotLanguageDirectory(gobEngine));
	}

	Common::InSaveFile* SaveReader::openSave() {
		return openSave(*_gobEngine, _fileName);
	}
	...
	bool SaveReader::getInfo(const GobEngine& gobEngine, const Common::String& fileName, SavePartInfo& info) {
		Common::InSaveFile* in = openSave(gobEngine, fileName);
	...
	Common::OutSaveFile* SaveWriter::openSave(const Common::String& fileName) {
		...
		return saveMan->openSaveForSaving(fileName);
	}
