...
namespace Gob {
	...
	bool SaveLoad_v3::GameHandler::load(int16 dataVar, int32 size, int32 offset) {
		...
		if (((uint32)offset) < kPropsSize) {
			...
			debug(SoftLevel::kGame, "Loading global properties");
	...
	bool SaveLoad_v3::GameHandler::save(int16 dataVar, int32 size, int32 offset) {
		...
		if (((uint32)offset) < kPropsSize) {
			...
			debug(SoftLevel::kGame, "Saving global properties");
	...
	bool SaveLoad_v3::GameHandler::loadScreenshot(int slot,
		...
		// WORKAROUND: issue #23100
		// Do not load the screenshot if the slot is locked !
		if (slot > _vm->_game->_highestReachedLevel) return false;
	...
	bool SaveLoad_v3::GameHandler::createReader(int slot) {
		...
		// WORKAROUNG for Goblins 3
		// Force recreate the reader for each slot - see issue #24257 for more information
		...
		if (converter.isOldSave()) {
			...
			_reader = new SaveReader(_vm, _usesScreenshots ? 3 : 2, slot, converter);
		}
		else _reader = new SaveReader(_vm, _usesScreenshots ? 3 : 2, slot, slotFile);
	...
	void SaveLoad_v3::ScreenshotHandler::File::buildScreenshotIndex(byte* buffer) const {
		...
		for (uint32 i = 0; i < _slotCount; i++, buffer++) {
			...
			if (Common::InSaveFile* in = saveMan->openFileOrSaveForLoading(LoadSavesFromBothRamAndSaves(*_vm), slotFile, GetSaveSlotLanguageDirectory(*_vm))) {
				delete in;
				*buffer = 1;
			}
		}
