...
namespace Gob {
	...
	bool SaveLoad_v4::GameHandler::createReader(int slot) {
		...
		if (!_reader || (_reader->getSlot() != ((uint32)slot))) {
			...
			if (converter.isOldSave()) {
				...
				_reader = new SaveReader(_vm, 3, slot, converter);

			}
			else
				_reader = new SaveReader(_vm, 3, slot, slotFile);
	...
	bool SaveLoad_v4::CurScreenPropsHandler::load(int16 dataVar,
		...
		debug(SoftLevel::kGame, "Loading screen properties (%d, %d, %d)",
			dataVar, size, offset);
	...
	bool SaveLoad_v4::CurScreenPropsHandler::save(int16 dataVar,
		...
		debug(SoftLevel::kGame, "Saving screen properties (%d, %d, %d)",
			dataVar, size, offset);
