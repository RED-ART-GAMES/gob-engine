...
namespace Gob {
	...
	bool SaveLoad_Playtoons::GameHandler::load(int16 dataVar, int32 size, int32 offset) {
		...
		if (size < 0) {
			...
			debug(SoftLevel::kGame, "Loading temporary sprite %d at pos %d", size, offset);
		...
		if (((uint32)offset) < kPropsSize) {
			...
			debug(SoftLevel::kFilesystem, "Loading from slot %d", slot);
			...
			reader = new SaveReader(_vm, 2, slot, slotFile);
	...
	bool SaveLoad_Playtoons::GameHandler::save(int16 dataVar, int32 size, int32 offset) {
		...
		if (size < 0) {
			...
			debug(SoftLevel::kGame, "Saving temporary sprite %d at pos %d", size, offset);
