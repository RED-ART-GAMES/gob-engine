...
namespace Gob {
	...
	*/
	...
	public:
		SaveReader(const GobEngine* const engine, uint32 partCount, uint32 slot, const Common::String& fileName);
		SaveReader(const GobEngine* const engine, uint32 partCount, uint32 slot, Common::SeekableReadStream& stream);
		~SaveReader();
		...
		/** Find and read the save's info part. */
		static bool getInfo(const GobEngine& gobEngine, const Common::String& fileName, SavePartInfo& info);

	protected:
		Common::String _fileName;
		const GobEngine* _gobEngine;
		Common::SeekableReadStream* _stream;

		bool _loaded;

		static Common::InSaveFile* openSave(const GobEngine& gobEngine, const Common::String& fileName);
