...
#define OPCODEGOB(i, x)   _opcodesGob[i]._OPCODEGOB(OPCODEVER, x)
	...
	static const std::map<std::string, Gob::Levels::Level> GOB2_MATCHING_LEVELS = {
		{ std::string("gob08"), Gob::Levels::Level::TWO },
		{ std::string("gob10"), Gob::Levels::Level::THREE },
		{ std::string("gob12"), Gob::Levels::Level::FOUR },
		{ std::string("gob14"), Gob::Levels::Level::FIVE },
		{ std::string("gob18"), Gob::Levels::Level::SIX },
		{ std::string("gob21"), Gob::Levels::Level::SEVEN },
		{ std::string("final"), Gob::Levels::Level::EIGHT },
	};

	static const std::map<std::string, Gob::Levels::Level> GOB3_MATCHING_LEVELS = {
		{ std::string("emap1002"), Gob::Levels::Level::TWO },
		{ std::string("emap1003"), Gob::Levels::Level::THREE },
		{ std::string("emap1004"), Gob::Levels::Level::FOUR },
		{ std::string("emap1007"), Gob::Levels::Level::FIVE },
		{ std::string("emap1010"), Gob::Levels::Level::SIX },
		{ std::string("emap1013"), Gob::Levels::Level::SEVEN },
		{ std::string("emap1015"), Gob::Levels::Level::EIGHT },
		{ std::string("emap1017"), Gob::Levels::Level::NINE },
		{ std::string("emap1019"), Gob::Levels::Level::TEN },
		{ std::string("final"),    Gob::Levels::Level::ELEVEN },
	};
	...
	void Inter_v2::o2_initMult() {
		...
		debug(SoftLevel::kDrawCalls, "o2_initMult: x = %d, y = %d, w = %d, h = %d",
			_vm->_mult->_animLeft, _vm->_mult->_animTop,
			_vm->_mult->_animWidth, _vm->_mult->_animHeight);
		debug(SoftLevel::kDrawCalls, "    _vm->_mult->_objCount = %d, "
	...
	void Inter_v2::o2_playCDTrack() {
		/*
		* NOTE(Florian): The inner `if` block used to be commented out, but this caused issues (cf SC26164).
		*                Why was this disabled? I couldn't find anything - but what I do know is that this
		*                is present in the base we started from and calling `blitInvalidated()` fixes
		*                SC26164.
		*
		*                cf 3029a91b77afd9c2511b1687e6045a887f0ed813 in the Gitlab repo.
		*/
		if (_vm->getGameType() == kGameTypeGob2 && (_vm->getFeatures() & kFeaturesCD)) {
	...
	void Inter_v2::o2_waitCDTrackEnd() {
		debug(SoftLevel::kAudio, "CDROM: Waiting for playback to end");
	...
	void Inter_v2::o2_totSub() {
		...
		// WORKAROUND: To know how and when to unlock a chapter, then we have to save the last level reached
		// No need to check for Gobliiins as we Gobliiins is not executed in inter_v2 (but inter_v1)
		const std::map<std::string, Gob::Levels::Level>* matchingLevels = (kGameTypeGob2 == _vm->getGameType() ? &GOB2_MATCHING_LEVELS : &GOB3_MATCHING_LEVELS);
		std::string _totFile = std::string(totFile.c_str());
		std::transform(_totFile.begin(), _totFile.end(), _totFile.begin(), ::tolower);
		if (const auto it = matchingLevels->find(_totFile); matchingLevels->end() != it && _vm->_game->_highestReachedLevel < it->second) {
			// Save the new level
			_vm->_game->_highestReachedLevel = it->second;
			// Unlock the achievement
			debug(kGame, "Trying to unlock achievement on game with type %d, level %d...", _vm->getGameType(), _vm->_game->_highestReachedLevel);
			g_system->getDataServiceManager()->unlockAchievement((uint32)_vm->_game->_highestReachedLevel);
			// WORKAROUND: reset the music cache once we change of chapter, for Gob2 & Gob3, CD-Rom version only
			if (_vm->getFeatures() & kFeaturesCD) {
				debug(kAudio, "Reset stream cache...");
				g_system->getAudioCDManager()->resetStreamsCache();
			}
		}
		...
		// For Gob3, if the operation to load automatically a save is performed, then shut down it
		if (Gob::GameType::kGameTypeGob3 == _vm->getGameType() && Gob::Game::LoadAutoState::DONE == _vm->_game->loadFirstSlotAutomatically)
		{
			debug(SoftLevel::kGame, "Disabling first slot load action");
	...
	void Inter_v2::o2_setGoblinState() {
		...
		case 11:
			...
			// See SC54080. We apply the same fix the US version has.
			{
				uint32_t takeOffset = 0;
				uint32_t _;

				if (_vm->totRequiresMayoHack(takeOffset, _) && _vm->_game->_script->pos() == takeOffset) {
					// "Don't play the animation again" flag common to all versions.
					_variables->writeOff32(5580, _variables->readOff32(236));

					constexpr uint32_t commonValue = 0;

					/*
					* Force to trigger the last branch of the subroutine the next time we enter it.
					* I tried matching the US version more closely, but it just straight up didn't
					* work.
					*/
					_variables->writeOff32(1860, commonValue);
					_variables->writeOff32(1880, commonValue);
				}
			}
	...
	void Inter_v2::o2_playImd() {
		...
		debug(SoftLevel::kVideoPlayer, "Playing video \"%s\" @ %d+%d, frames %d - %d, "
	...
	void Inter_v2::o2_openItk() {
		Common::String file = _vm->_game->_script->evalString();
		// Check ticket #22394 for more information about this switch
		if (_vm->_language == Common::Language::ES_ESP && _vm->getGameType() == kGameTypeGob3 && _vm->getFeatures() & kFeaturesCD && file.matchString("FRGOB3")) {
			file = "USGOB3";
		}
	...
	void Inter_v2::o2_assign(OpFuncParams& params) {
		const int32_t positionAtStartOfAssignment = _vm->_game->_script->pos();
		...
		if (positionAtStartOfAssignment == _vm->_assignSpeakerOnHookPosition) {
			executeSpeakerOnAssignmentHook();
		}
	}

	void Inter_v2::executeSpeakerOnAssignmentHook() {
		/*
		* Note(Florian): Footstep sounds are  missing in floppy editions of Gob2.   Comparing the game
		*                script in  the CD edition  with the floppy edition's  led me to a  particular
		*                routine which exists in both editions,  but was  called in a specific spot in
		*                the CD but not in floppy versions.    Since calling procedures messes up with
		*                the  state  of the bytecode  interpreter,   I decided to just reimplement the
		*                script because it's always the same in all levels of Goblins 2  (the function
		*                location and the hooked assignment notwithstanding).
		*
		* cf. SC12920
		*/
		do {
			uint8_t var8_9740 = _variables->readOff8(9740);

			if (var8_9740 != 0) {
				break;
			}

			uint32_t var32_12740 = _variables->readOff32(12740);

			if (var32_12740 <= 0 || var32_12740 >= 320) {
				break;
			}

			uint32_t var32_0236_and_var8_9741 = _variables->readOff8(9741);
			uint32_t var32_1860_and_var8_9742 = _variables->readOff8(9742);

			_variables->writeOff32(236,   var32_0236_and_var8_9741);
			_variables->writeOff32(1860,  var32_1860_and_var8_9742);
			_variables->writeOff32(14996, 0);

			if (var32_0236_and_var8_9741 < 0 || var32_0236_and_var8_9741 > 19) {
				break;
			}

			if (var32_1860_and_var8_9742 != 2 && var32_1860_and_var8_9742 != 5) {
				break;
			}

			_vm->_sound->speakerOn(50, 5);

			_variables->writeOff32(14996, 1);
		} while (false);

		{
			uint8_t var8_9780 = _variables->readOff8(9780);

			if (var8_9780 != 0) {
				return;
			}

			uint32_t var32_12744 = _variables->readOff32(12744);

			if (var32_12744 <= 0 || var32_12744 >= 320) {
				return;
			}

			uint32_t var32_0236_and_var8_9781 = _variables->readOff8(9781);
			uint32_t var32_1860_and_var8_9782 = _variables->readOff8(9782);

			_variables->writeOff32(236,  var32_0236_and_var8_9781);
			_variables->writeOff32(1860, var32_1860_and_var8_9782);

			if (var32_0236_and_var8_9781 < 0 || var32_0236_and_var8_9781 > 19) {
				return;
			}

			if (var32_1860_and_var8_9782 != 2 && var32_1860_and_var8_9782 != 5) {
				return;
			}

			if (_variables->readOff32(14996) != 0) {
				return;
			}
	...
	void Inter_v2::o2_printText(OpFuncParams& params) {
		...
		do {
			...
			if (_vm->_game->_script->peekByte() != 200) {
				...
				case TYPE_ARRAY_STR:
				{
					...
					Common::String currentTotFileLC = _vm->_game->_curTotFile;
					currentTotFileLC.toLowercase();
				}
				break;
				...
				// Feature #24340
				// Replace copyright information of Gob2 with RAG copyright
				if (Gob::GameType::kGameTypeGob2 == _vm->getGameType()) {
					const char* REPLACEMENT_STRS[] = { (const char*)"COKTEL VISION" };
					for (const char* str : REPLACEMENT_STRS) {
						if (char* detectedSubString = strstr(buf + i, str); detectedSubString) {
							memcpy(detectedSubString, (void*)"RED ART GAMES", strlen(str));
						}
					}
				}
				// Feature #24329
				// Replace copyright information of Gob3 with RAG copyright
					...
					if ('V' == *(buf + i)) {
						...
						for (const char* str : REPLACEMENT_STRS) {
							if (char* detectedSubString = strstr(buf + i, str); detectedSubString) {
								if (strlen(str) == 3) { memcpy(detectedSubString, (void*)"RAG", 3); }         // "MDO"
								else if (strlen(str) == 6) { memcpy(detectedSubString, (void*)"R.A.G.", 6); } // "SIERRA"
								else { warning("Cannot replace substring with '%s' : undetected length comparison to replace the \"VERSION\" string in the game!", detectedSubString); }
							}
						}

						const char* YEAR_REPLACEMENT_STRS[] = {
							(const char*)"1993",
							(const char*)"1994"
						};
						for (const char* str : YEAR_REPLACEMENT_STRS) {
							if (char* detectedSubString = strstr(buf + i, str); detectedSubString) {
								memcpy(detectedSubString, (void*)"2025", 4); // Force replacement to publishing year
							}
						}
					}
				}
		...
		if (_vm->_game->secondStepForAutoSave)
		{
			Common::Event event;
			...
			_vm->_game->secondStepForAutoSave = false;
		}
	...
	void Inter_v2::o2_goblinFunc(OpFuncParams& params) {
		...
		//
		// This unpause the game after a **LOAD** (not a SAVE)!!
		if (Gob::GameType::kGameTypeGob2 == _vm->getGameType() && _vm->_game->isPlayerStatePaused() && (Gob::Game::LoadAutoState::DONE == _vm->_game->loadFirstSlotAutomatically)) {
			// For Gob2 and Gob3, if the operation to load automatically a save is performed, then shut down it
			debug(SoftLevel::kGame, "Disabling first slot load action");
			_vm->_game->loadFirstSlotAutomatically = Gob::Game::LoadAutoState::NOT_PLANNED;
	...
	void Inter_v2::o2_checkData(OpFuncParams& params) {
		...
		debug(SoftLevel::kFilesystem, "Requested size of file \"%s\": %d", file.c_str(), size);
	...
	void Inter_v2::o2_readData(OpFuncParams& params) {
		...
		debug(SoftLevel::kFilesystem, "Read from file \"%s\" (%d, %d bytes at %d)",
		...
		if (mode == SaveLoad::kSaveModeSave) {
			g_system->getSavefileManager()->mountSaveData();
			WRITE_VAR(1, 1);
			if (!_vm->_saveLoad->load(file, dataVar, size, offset)) warning("Failed to load saved game from file.");
			else WRITE_VAR(1, 0);
			g_system->getSavefileManager()->umountSaveData();
			return;
		}
	...
	void Inter_v2::saveGobFile(const char* const level) {
		/** FOR CONSOLES */
		// The behaviour of Gob3 is different than Gob2:
		// * Gob3 saves at launch the game (automatically), which results in skipping automatically the credits scene in the next game start.
		// * Gob2 does not save at launch the game, and does not perform any automated ways to save the data without user's explicit permission (a click)
		//	 so relaunching Gob2 right after does not skip automatically the credits scene...
		// For the moment we can consider this as an "okay behaviour", but the most simplest thing to do is to perform like Gob3: at start, check if the 
		// save exists. If not, then create it, and it will automatically skip the credits scene next time **even if** the user did not explicitly saved the game.
	...
	void Inter_v2::o2_writeData(OpFuncParams& params) {
		...
		debug(SoftLevel::kFilesystem, "Write to file \"%s\" (%d, %d bytes at %d)",
			file, dataVar, size, offset);
...
#if _HAS_EXCEPTIONS
				throw std::exception("Failed to save game to file.");
#endif
			}
			else {
				_vm->_saveLoad->commitChanges(file);

				WRITE_VAR(1, 0);

				// There used to be an unexplained difference between Gob2 and 3 here for some reason. It seems invalid.
				_vm->_game->_highestReachedLevel = static_cast<Gob::Levels::Level>(std::max(std::min(static_cast<int>(_vm->_game->_highestReachedLevel), 20), 1));

				const Common::String levelAsString = (
					_vm->getGameType() == kGameTypeGob2
					? Gob::Levels::Gob2::getLevelIdStringFrom(_vm->_game->_highestReachedLevel)
					: Gob::Levels::Gob3::getLevelIdStringFrom(_vm->_game->_highestReachedLevel));

				saveGobFile(levelAsString.c_str());
				/** FOR CONSOLES */
			}
			g_system->getSavefileManager()->umountSaveData();
		}
