...
#define OPCODEGOB(i, x)   _opcodesGob[i]._OPCODEGOB(OPCODEVER, x)
	...
	void Inter_Playtoons::oPlaytoons_checkData(OpFuncParams& params) {
		...
		debug(SoftLevel::kFilesystem, "Requested size of file \"%s\": %d",
			file.c_str(), size);
...
#if _HAS_EXCEPTIONS
				throw std::exception("Failed to load saved game from file.");
#endif
			}
			else
				WRITE_VAR(1, 0);
	...
	Common::String Inter_Playtoons::getFile(const char* path, bool stripPath) {
		...
		const Common::String origS = Common::String(orig);

		if (origS != newPath)
			debug(SoftLevel::kFilesystem, "Inter_Playtoons::getFile(): Evaluating path"
				"\"%s\" to \"%s\"", origS.c_str(), path);
