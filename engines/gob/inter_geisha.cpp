...
#define OPCODEGOB(i, x)   _opcodesGob[i]._OPCODEGOB(OPCODEVER, x)
	...
	void Inter_Geisha::oGeisha_readData(OpFuncParams& params) {
		...
		debug(SoftLevel::kFilesystem, "Read from file \"%s\" (%d)", file, dataVar);
...
#if _HAS_EXCEPTIONS
				throw std::exception((char*)"Failed to load saved game from file.");
#endif
			}
			else
				WRITE_VAR(1, 0);
	...
	void Inter_Geisha::oGeisha_writeData(OpFuncParams& params) {
		...
		debug(SoftLevel::kFilesystem, "Write to file \"%s\" (%d, %d bytes)", file, dataVar, size);
		...
		if (mode == SaveLoad::kSaveModeSave) {
			...
			if (!_vm->_saveLoad->save(file, dataVar, size, 0)) {
				...
				//GUI::MessageDialog dialog(Common::U32String("Failed to save game to file."));
				//dialog.runModal();
#if _HAS_EXCEPTIONS
