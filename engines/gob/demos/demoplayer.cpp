...
namespace Gob {
	...
	bool DemoPlayer::play(const char* fileName) {
		...
		debug(SoftLevel::kFilesystem, "Playing \"%s\"", fileName);
	...
	bool DemoPlayer::play(uint32 index) {
		...
		debug(SoftLevel::kFilesystem, "Playing demoIndex %d: %d", index, script.source);
	...
	void DemoPlayer::clearScreen() {
		debug(SoftLevel::kDrawCalls, "Clearing the screen");
	...
	void DemoPlayer::playVideo(const char* fileName) {
		...
		debug(SoftLevel::kFilesystem, "Playing video \"%s\"", file);
	...
	void DemoPlayer::playVideoDoubled(int slot) {
		...
		for (uint i = 0; i < _vm->_vidPlayer->getFrameCount(slot); i++) {
			...
			const Common::List<Common::Rectangle>* rects = _vm->_vidPlayer->getDirtyRects(slot);
			if (rects) {
				for (Common::List<Common::Rectangle>::const_iterator rect = rects->begin(); rect != rects->end(); ++rect) {
	...
	void DemoPlayer::playADL(const Common::String& fileName, bool waitEsc, int32 repeat) {
		debug(SoftLevel::kFilesystem, "Playing ADL \"%s\" (%d, %d)", fileName.c_str(), waitEsc, repeat);
	...
	void DemoPlayer::evaluateVideoMode(const char* mode) {
		debug(SoftLevel::kVideoPlayer, "Video mode \"%s\"", mode);
