...
namespace Gob {
	...
	bool SCNPlayer::readLabels(Common::SeekableReadStream& scn, LabelMap& labels) {
		debug(SoftLevel::kVideoPlayer, "Reading SCN labels");
		...
		while (!scn.err() && !scn.eos()) {
			...
			if (lineStartsWith(line, "LABEL ")) {
				...
				debug(SoftLevel::kVideoPlayer, "Found label \"%s\" (%d)", line.c_str() + 6, (int)scn.pos());
			}
		}
	...
	void SCNPlayer::gotoLabel(Common::SeekableReadStream& scn,
		...
		debug(SoftLevel::kVideoPlayer, "Jumping to label \"%s\"", label);
