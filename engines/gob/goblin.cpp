...
#include "gob/hotspots.h"
 ...
 // To display objects as hotspots, for Gobliiins 1!
...
namespace Gob {
	...
	void Goblin::drawObjects() {
		...
		for (ptr = _objList->pHead; ptr != nullptr; ptr = ptr->pNext) {
			...
			// RAG: code below removed according to https://github.com/scummvm/scummvm/commit/66190d8030ed4f0b02f58a2cde2d9439bc51780e
			// this fix visual artifacts we had in Gob1 (SC43782)
			//objDesc->dirtyLeft = 0;
		...
		// Gobliins1 has special properties compared to Gobliins 2 and 3, as
		// hotspots can be contained in objects!
		// We must update both the "official" list of hotspots (see hotspots.cpp) but
		// also consider each object has a potential hotspot too.
		// This is only used for Gobliiins1, no need for other games.
		if (_vm->getGameType() == Gob::GameType::kGameTypeGob1) {
			updateHotspotsGob1();
		}
	}
	...
	void Goblin::switchGoblin(int16 index) {
		...
		debug(SoftLevel::kGame, "switchGoblin");
		if (VAR(59) != 0)
			return;
	...
	void Goblin::updateHotspotsGob1() {
		GraphicsManager* manager = g_system->getGraphicsManager();
		if (NULL == manager) return; // Should not happen normally...

		manager->clearObjectCheatsList();

		for (int i = 0; i < _objCount; ++i) {
			if (NULL == _objects[i]) continue;
			const std::string levelName = std::string(_vm->_game->_curTotFile.c_str());
			if (!_vm->_game->_managedCheats->canDisplay(levelName, Gob::ManagedCheats::Kind::kObject, (uint16)i)) continue;
			if ((_objects[i]->left != _objects[i]->right) && (_objects[i]->top != _objects[i]->bottom)) {
				int16 offset_x = _vm->_game->_managedCheats->getObjectOffsetX(levelName, Gob::ManagedCheats::Kind::kObject, (uint16)i);
				int16 offset_y = _vm->_game->_managedCheats->getObjectOffsetY(levelName, Gob::ManagedCheats::Kind::kObject, (uint16)i);

				Common::Point topLeft(_objects[i]->left, _objects[i]->top);
				Common::Point bottomRight(_objects[i]->right, _objects[i]->bottom);
				manager->addObjectCheat((uint16)i, _vm->_game->_curTotFile, topLeft, bottomRight, offset_x, offset_y);
			}
		}
	}
