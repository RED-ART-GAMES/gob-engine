...
namespace Gob {
	...
	public:
		...
		void clear();
		void allowToDisplay(const std::string& levelName, const ManagedCheats::Kind kind, const uint16 id, const int16 x_offset, const int16 y_offset);
		bool canDisplay(const std::string& levelName, const ManagedCheats::Kind kind, const uint16 id);
		uint16 getObjectOffsetX(const std::string& levelName, const ManagedCheats::Kind kind, const uint16 id);
		uint16 getObjectOffsetY(const std::string& levelName, const ManagedCheats::Kind kind, const uint16 id);
		void addCustomCheat(const std::string& levelName, const Common::String& id, const Common::Point& topLeft, const Common::Point& bottomRight);
		// Constant reference, avoid to change by default!!
		// This is only a getter, shouldn't be used as a setter...
		bool existsCustomCheatsFor(const std::string& levelName);
		std::vector<std::tuple<Common::String, Common::Point, Common::Point>>& getCustomCheats(const std::string& levelName);

	private:
		std::map<std::string, std::vector<std::tuple<Kind, uint16, int16, int16>>> _cheatsToDisplay;
		std::map<std::string, std::vector<std::tuple<Common::String, Common::Point, Common::Point>>> _customCheats;
	};
