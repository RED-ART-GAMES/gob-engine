...
namespace Gob {
		...
		while (true) {
			...
			if ((operation >= OP_ARRAY_INT8) && (operation <= OP_FUNC)) {
				...
				case OP_LOAD_VAR_INT16: // int16 variable load
					debug(SoftLevel::kInterpreter, "var16_%d", _vm->_game->_script->readUint16());
					break;

				case OP_LOAD_VAR_INT8: // int8 variable load:
					debug(SoftLevel::kInterpreter, "var8_%d", _vm->_game->_script->readUint16());
					break;

				case OP_LOAD_IMM_INT32: // int32/uint32 immediate
					debug(SoftLevel::kInterpreter, "%d", _vm->_game->_script->readInt32());
					break;

				case OP_LOAD_IMM_INT16: // int16 immediate
					debug(SoftLevel::kInterpreter, "%d", _vm->_game->_script->readInt16());
					break;

				case OP_LOAD_IMM_INT8: // int8 immediate
					debug(SoftLevel::kInterpreter, "%d", _vm->_game->_script->readInt8());
					break;

				case OP_LOAD_IMM_STR: // string immediate
					debug(SoftLevel::kInterpreter, "\42%s\42", _vm->_game->_script->readString());
					break;

				case OP_LOAD_VAR_INT32:
				case OP_LOAD_VAR_INT32_AS_INT16:
					debug(SoftLevel::kInterpreter, "var_%d", _vm->_game->_script->readUint16());
					break;
				...
				case OP_ARRAY_STR:   // string array access
					...
					debug(SoftLevel::kInterpreter, "var_%d[", _vm->_game->_script->readInt16());
					...
					for (dim = 0; dim < dimCount; dim++) {
						...
						debug(SoftLevel::kInterpreter, " of %d", (int16)arrDesc[dim]);
						if (dim != dimCount - 1)
							debug(SoftLevel::kInterpreter, ",");
					}
					debug(SoftLevel::kInterpreter, "]");
				...
				case OP_FUNC: // function
					...
					if (func == FUNC_SQR)
						debug(SoftLevel::kInterpreter, "sqr(");
					else if (func == FUNC_RAND)
						debug(SoftLevel::kInterpreter, "rand(");
					else if (func == FUNC_ABS)
						debug(SoftLevel::kInterpreter, "abs(");
					else if ((func == FUNC_SQRT1) || (func == FUNC_SQRT2) || (func == FUNC_SQRT3))
						debug(SoftLevel::kInterpreter, "sqrt(");
					else
						debug(SoftLevel::kInterpreter, "id(");
			...
			case OP_END_MARKER:
				...
				if (stopToken != OP_END_MARKER) {
					debug(SoftLevel::kInterpreter, "Closing paren without opening?");
				}
				break;

			default:
				debug(SoftLevel::kInterpreter, "<%d>", (int16)operation);
	...
	void Expression::printVarIndex() {
		...
		case OP_LOAD_VAR_STR:
			...
			debug(SoftLevel::kInterpreter, "&var_%d", temp);
		...
		default:
			debug(SoftLevel::kInterpreter, "var_0");
			break;
		}
		debug(SoftLevel::kInterpreter, "\n");
	...
	bool Expression::getVarBase(uint32& varBase, bool mindStop,
		...
		while ((operation == 14) || (operation == 15)) {
			...
			if (operation == 14) {
				...
				debug(SoftLevel::kInterpreter, "varBase: %d, by %d", varBase, operation);
	...
	uint16 Expression::parseVarIndex(uint16* size, uint16* type) {
		...
		debug(SoftLevel::kInterpreter, "var parse = %d", operation);
		switch (operation) {
		case OP_ARRAY_INT8:
		case OP_ARRAY_INT32:
		case OP_ARRAY_INT16:
		...
		case OP_LOAD_VAR_STR:
			temp = _vm->_game->_script->readUint16() * 4;
			debug(SoftLevel::kInterpreter, "oper = %d", _vm->_game->_script->peekInt16());
			if ((operation == OP_LOAD_VAR_STR) && (_vm->_game->_script->peekByte() == 13)) {
				_vm->_game->_script->skip(1);
				val = parseValExpr(OP_END_MARKER);
				temp += val;
				debug(SoftLevel::kInterpreter, "parse subscript = %d", val);
			}
			return varBase + temp;
