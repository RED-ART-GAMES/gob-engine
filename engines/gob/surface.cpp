...
namespace Gob {
	...
	Pixel::Pixel(byte* vidMem, uint8 bpp, byte* min, byte* max) :
		_vidMem(vidMem), _min(min), _max(max), _bpp(bpp) {
	...
	Common::Rectangle Surface::fillAreaAtPoint(int16 left, int16 top, uint32 fillColor) {
		Common::Rectangle modifiedArea;
		...
		pixel.set(fillColor);
		modifiedArea.extendToContainOtherRectangle(Common::Rectangle(left, top, left + 1, top + 1));
			...
			for (int i = 0; i < 4; i++) {
				...
				if (p.get() == initialColor) {
					...
					if (!modifiedArea.contains(x, y))
						modifiedArea.extendToContainOtherRectangle(Common::Rectangle(x, y, x + 1, y + 1));
	...
	void GraphicsdrawLine(uint16 x0, uint16 y0, uint16 x1, uint16 y1, uint32 color, void (*func)(int, int, int, void*), void* data)
	{
		int dx = x1 - x0;
		int dy = y1 - y0;
		...
		for (int i = 0; i <= steps; i++) {
			func(round(x), round(y), color, data);
			x += x_inc;
			y += y_inc;
		}
	}
	...
	void Surface::drawLine(uint16 x0, uint16 y0, uint16 x1, uint16 y1, uint32 color) {
		GraphicsdrawLine(x0, y0, x1, y1, color, &plotPixel, this);
	}
