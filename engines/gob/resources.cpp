...
namespace Gob {
	...
	byte* Resources::loadTOTLocTexts(const Common::String& fileBase, int32& size) {
		...
		debug(SoftLevel::kGame, "Using language %d for %s",
	...
	Resource* Resources::getTOTResource(uint16 id) const {
		...
		/*
		* "Try again" dialog size. If there is something else of that size, we're basically screwed.
		* As far as we know, though, that's the only resource of that size.
		*
		* If we get this wrong, it's not too bad, because this will get reset when we evaluate
		* hotspots.
		*
		* The value stored here will be garbage for Gob23, so it's fine to set regardless of
		* what actual game we're playing.
		*/
		if (totItem.width == 98 && totItem.height == 72) {
			_vm->_gob1_isShowingGameRetryDialog = true;
		}
	...
	Resource* Resources::getEXTResource(uint16 id) const {
		...
		/*
		* Full screen resource - likely to cover everything and should therefore mean that
		...
		* what actual game we're playing.
		*/
		_vm->_gob1_isShowingGameRetryDialog = extItem.width == 320 && extItem.height == 200;
	...
	byte* Resources::getEXData(EXTResourceItem& extItem, uint32& size) const {
		...
		byte* data = new byte[extItem.packed ? (size + 2) : size];
		const int streamReadBytes = stream->read(data, size);
		if (streamReadBytes != size) {
			warning("Expected to read %d bytes, but readed %d bytes instead...", size, streamReadBytes);
		}
		// FIXME !!!
		// For Gob1, it seems there is an issue reading data from level 19, which leads to missing data
		// and unfinished game (Gob1 **Windows version** only)
		// This only happens for one game object, and the read size is different than the size of load (lower)
		// So, bypass this error as soon as we reach this **for the exact condition of the game**
		// This particular object is for Gob1 windows, packed set to True, EX resource, and at offset '110502'
		if ((streamReadBytes != size) &&
			!((_vm->getFeatures() & kFeaturesAdLib) && (Gob::GameType::kGameTypeGob1 == _vm->getGameType()) && 
				extItem.packed && (extItem.type == kResourceEX) && (extItem.offset = 110502)
			))
		{
			delete[] data;
			delete stream;
