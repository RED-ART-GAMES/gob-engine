...
namespace Gob {
	...
	bool Sound::sampleLoad(SoundDesc* sndDesc, SoundType type, const char* fileName) {
		...
		info("Loading sample \"%s\"", fileName);
	...
	void Sound::speakerOn(int16 frequency, int32 length) {
		...
		debug(SoftLevel::kAudio, "PCSpeaker: Playing tone (%d, %d)", frequency, length);

		// For Gob1, entering the password, there is a BEEP-BEEP sound
		// each time we enter a character.
		// This is quite annoying, and we want to get rid off that.
		// The frequency is 5_000, but we want to be sure that the frequency is not used
		// by any other functions!
		// If the 5_000 frequency is only reserved for entering the password in Gob1, then
		// we could write a condition to speak the speakerOn at this specific frequency.
		// Otherwise, we have to reverse engineer the script code to bypass the speakers as soon as
		// the password hotspot (check for inputs) losed focus (leave?)...
		if ((Gob::kGameTypeGob1 == _vm->getGameType()) && 5000 == frequency) {
			warning(">>> DISABLED sound on Gobliiins (with frequency 5000) ! <<<");
			return;
		}
	...
	void Sound::speakerOff() {
		...
		debug(SoftLevel::kAudio, "PCSpeaker: Stopping tone");
	...
	bool Sound::infogramesLoadInstruments(const char* fileName) {
		// I removed Infogrames bc the format files associated are not present in our assets (.ins and .dum)
		error("oh, infogrames is needed?");
		return false;
		//if (!_infogrames)
		//	return false;
	...
	bool Sound::adlibLoadADL(const char* fileName) {
		...
		if (!_adlPlayer) _adlPlayer = new ADLPlayer(_vm);

		info("AdLib: Loading ADL data (\"%s\")", fileName);
	...
	bool Sound::adlibLoadADL(byte* data, uint32 size, int index) {
		...
		if (!_adlPlayer)
			_adlPlayer = new ADLPlayer(_vm);

		info("AdLib: Loading ADL data (%d)", index);
	...
	bool Sound::adlibLoadMDY(const char* fileName) {
		...
		debug(SoftLevel::kAudio, "AdLib: Loading MDY data (\"%s\")", fileName);
	...
	bool Sound::adlibLoadTBR(const char* fileName) {
		...
		debug(SoftLevel::kAudio, "AdLib: Loading MDY instruments (\"%s\")", fileName);
	...
	void Sound::adlibPlayBgMusic() {
		if (!_hasAdLib || !_hasAdLibBg) return; // If one of those is disabled, then stop there
		
		if (_adlPlayer && _adlPlayer->isPlaying())
			return;

		// Only accessible for Gob1 Windows version
		if (_vm->getGameType() != Gob::GameType::kGameTypeGob1 || _vm->getPlatform() != Common::Platform::kPlatformWindows || _vm->_silenceAdlibInGob1Windows)
			return;
		...
		if (_vm->getTracksPlatform() == Common::kPlatformWindows)
	...
	void Sound::adlibPlay() {
		...
		debug(SoftLevel::kAudio, "AdLib: Starting playback");
	...
	void Sound::adlibStop() {
		...
		debug(SoftLevel::kAudio, "AdLib: Stopping playback");
	...
	void Sound::blasterPlay(SoundDesc* sndDesc, int16 repCount,
		...
		debug(SoftLevel::kAudio, "SoundBlaster: Playing sample (%d, %d, %d)",
	...
	void Sound::blasterStop(int16 fadeLength, SoundDesc* sndDesc) {
		...
		debug(SoftLevel::kAudio, "SoundBlaster: Stopping playback");
	...
	void Sound::blasterPlayComposition(const int16* composition, int16 freqVal,
		...
		debug(SoftLevel::kAudio, "SoundBlaster: Playing composition (%d, %d)",
			freqVal, sndCount);
	...
	void Sound::blasterStopComposition() {
		...
		debug(SoftLevel::kAudio, "SoundBlaster: Stopping composition");
	...
	void Sound::blasterWaitEndPlay(bool interruptible, bool stopComp) {
		...
		debug(SoftLevel::kAudio, "SoundBlaster: Waiting for playback to end");
	...
	void Sound::cdLoadLIC(const Common::String& fname) {
		...
		debug(SoftLevel::kAudio, "CDROM: Loading LIC \"%s\"", fname.c_str());
	...
	void Sound::cdUnloadLIC() {
		...
		debug(SoftLevel::kAudio, "CDROM: Unloading LIC");
	...
	void Sound::cdPlayBgMusic() {
		...
		for (int i = 0; i < ARRAYSIZE(tracks); i++)
			if (_vm->isCurrentTot(tracks[i][0])) {
				debug(SoftLevel::kAudio, "CDROM: Playing background music \"%s\" (\"%s\")", tracks[i][1], tracks[i][0]);
	...
	void Sound::cdPlayMultMusic() {
		...
		for (int i = 0; i < ARRAYSIZE(tracks); i++)
			if (_vm->isCurrentTot(tracks[i][0])) {
				debug(SoftLevel::kAudio, "CDROM: Playing mult music \"%s\" (\"%s\")", tracks[i][language + 1], tracks[i][0]);
	...
	void Sound::cdPlay(const Common::String& trackName) {
		if (!_cdrom)
			return;
		info("CDROM: Playing track \"%s\"", trackName.c_str());
	...
	void Sound::cdStop() {
		...
		debug(SoftLevel::kAudio, "CDROM: Stopping playback");
		_cdrom->stopPlaying();
	}
	...
	void Sound::bgPlay(const char* file, SoundType type) {
		...
		info("BackgroundAtmosphere: Playing \"%s\"", file);
	...
	void Sound::bgPlay(const char* base, const char* ext, SoundType type, int count) {
		...
		info("BackgroundAtmosphere: Playing \"%s\" (%d)", base, count);
	...
	void Sound::bgStop() {
		...
		debug(SoftLevel::kAudio, "BackgroundAtmosphere: Stopping playback");
	...
	void Sound::bgShade() {
		...
		debug(SoftLevel::kAudio, "BackgroundAtmosphere: Shading playback");
	...
	void Sound::bgUnshade() {
		...
		debug(SoftLevel::kAudio, "BackgroundAtmosphere: Unshading playback");
	...
	void Sound::createADLPlayer() {
		...
		_adlPlayer = new ADLPlayer(_vm);
		// Infinite repetition of tracks by default
		// TODO: check if only needed for Gob1
		adlibSetRepeating(-1);
	}
