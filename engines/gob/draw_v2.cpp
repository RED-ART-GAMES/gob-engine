...
namespace Gob {
	...
	void Draw_v2::printTotText(int16 id) {
		...
		// In EN_US, numbers are reversed but the text itself is reversed too, so it's fine.
		// In the GRB version, the text is reversed but the number is not.
		if (
			(_vm->getGameType() == kGameTypeGob2)
			&& !_vm->isCD()
			&& _vm->isCurrentTot("gob07.tot")
			&& (
				_vm->_language == Common::Language::EN_ANY
				|| _vm->_language == Common::Language::EN_GRB)
		) {
	...
	// Replacement of strings in MENU.TOT, issue #24073
	static const char* kNotePadStrings[][2] = {
		{"NOTEPAD"    , "CLUES"},
		{"NOTE-PAD"   , "CLUES"},
		{"BLOC-NOTES" , "INDICES"},
		{"BLOC-NOTE"  , "INDIZE"},
	...
	void Draw_v2::replaceNotePadStrings() {
		if (!_textToPrint || (_vm->getGameType() != kGameTypeGob2) || (!_vm->isCurrentTot("MENU.TOT"))) return;

		for (int i = 0; i < ARRAYSIZE(kNotePadStrings); i++) {
			if (!strcmp(_textToPrint, kNotePadStrings[i][0])) {
				_textToPrint = kNotePadStrings[i][1];
				return;
			}
		}
	}
