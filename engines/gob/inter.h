...
#define CLEAROPCODEGOB(i)  _opcodesGob.erase(i)
	...
	public:
		...
		/// <summary>
		/// Stop and relaunch the game without terminating the app (VM)
		/// </summary>
		bool _relaunchGame = false;
		...
		virtual void saveGobFile(const char* const level)
		{}
	...
	public:
		...
		static std::optional<uint8_t> tryExtractGob2LevelNumber(const Common::String& totFile);

		void saveGobFile(const char* const level) override;
	...
	protected:
		...
		void speakerOn(const int16 frequency);

	protected:
		/// <summary>
		/// Workaround for missing footsteps in Gob2
		/// </summary>
		bool _ignoreSpeakerOff = false;

	private:
#pragma region Special RAG Functions
		// SPECIAL RAG FUNCTIONS
		/// <summary>
		/// Reload the current tot file
		/// </summary>
		void o1_override_reloadTot(OpFuncParams& params);
		/// <summary>
		/// Force reload of current tot
		/// </summary>
		/// <param name="params"></param>
		void o1_override_drawOperations(OpFuncParams& params);
		/// <summary>
		/// Performs a check each frame in order to know if the player clicked on "Load" button
		/// This function must performs this check at a specific offset, for the current version
		/// This function can change the offset depending of the situation (as an example, for
		/// calling "Again" for the "Load" button at level 1)
		/// </summary>
		/// <param name="params"></param>
		/// <param name="gameOffset">The current position in the game script</param>
		/// <returns>true if the current script function should stop being executed, false otherwise</returns>
		bool ox_checkForSaveAndLoadMecaGob1(OpFuncParams& params, uint16& gameOffset);
		/// <summary>
		/// Performs a check each frame in order to know if the player clicked on "Save", "Load", or "Version" button
		/// This function must performs this check at a specific offset, for the current version
		/// </summary>
		/// <param name="params"></param>
		/// <param name="gameOffset">The current position in the game script</param>
		/// <returns>true if the current script function should stop being executed, false otherwise</returns>
		bool ox_checkForSaveAndLoadMecaGob2(OpFuncParams& params, const uint16 gameOffset);
		/// <summary>
		/// Performs a check each frame in order to know if the player clicked on "Save", "Load", or "Version" button
		/// This function must performs this check at a specific offset, for the current version
		/// </summary>
		...
		/// <param name="gameOffset">The current position in the game script</param>
		/// <returns>true if the current script function should stop being executed, false otherwise</returns>
		bool ox_checkForSaveAndLoadMecaGob3(OpFuncParams& params, const uint16 gameOffset);
		/// <summary>
		/// Manage to load the first slot automatically for Gobliins 2 (Floppy-Disc & CD-ROM versions)
		/// </summary>
		/// <param name="params">unused, but needed for the interface</param>
		/// <param name="gameOffset">The current position in the game script</param>
		void ox_loadFirstSlotGob2(OpFuncParams& params, const uint16 gameOffset);
		/// <summary>
		/// Manage to load the first slot automatically for Gobliins 3 (Floppy Disc version)
		/// </summary>
		/// <param name="params">unused, but needed for the interface</param>
		/// <param name="gameOffset">The current position in the game script</param>
		void ox_loadFirstSlotGob3Floppy(OpFuncParams& params, const uint16 gameOffset);
		/// <summary>
		/// Manage to load the first slot automatically for Gobliins 3 (CD-ROM version)
		/// </summary>
		/// <param name="params">unused, but needed for the interface</param>
		/// <param name="gameOffset">The current position in the game script</param>
		void ox_loadFirstSlotGob3CD(OpFuncParams& params, const uint16 gameOffset);

		/// Search in preloaded .STK files for Gobliiins Floppy-disc version
		void o1_manageDataFileGob1Floppy();
		/// Search in preloaded .STK files for Gobliins 2 Floppy-disc version
		void o1_manageDataFileGob2Floppy();
		/// Search in preloaded .STK files for Goblins 3 Floppy-disc version
		void o1_manageDataFileGob3Floppy();
#pragma endregion Special RAG Functions
	};
	...
	private:
		void executeSpeakerOnAssignmentHook();
	};
