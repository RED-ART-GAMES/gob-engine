...
namespace Gob {
	...
	void Video::retrace(bool mouse) {
		...
		if (_vm->_global->_primarySurfDesc) {
			...
			// Only refresh the screen **if and only if** the game is not saving!
			// The screen is refreshed when the first slot is loaded automatically at startup, but we need to
			// pass the information to the Graphics manager that **we don't want to copy the interpreter framebuffer**
			// Also, if we want to draw something (like a LOADING screen), we want to pass the information to ImGui...
			if ((!_vm->_game->isPlayerStatePaused()) || (Gob::Game::LoadAutoState::NOT_PLANNED != _vm->_game->loadFirstSlotAutomatically)) g_system->updateScreen(!_vm->_game->isPlayerStatePaused());
			else { debug(kInterpreter, "saving on default slot: do not draw..."); }
		}
	}
	...
	void Video::dirtyRectsAdd(int16 left, int16 top, int16 right, int16 bottom) {
		...
		_dirtyRects.push_back(Common::Rectangle(left, top, right + 1, bottom + 1));
	}
	...
	void Video::dirtyRectsApply(int left, int top, int width, int height, int x, int y) {
		...
		Common::List<Common::Rectangle>::const_iterator it;
