...
namespace Gob {
	...
	void Goblin_v7::initiateMove(Mult::Mult_Object* obj) {
		...
		if (obj->goblinX != obj->gobDestX || obj->goblinY != obj->gobDestY) {
			debug(SoftLevel::kInterpreter, "Computing Obj %s new state (obj->goblinX = %d, obj->gobDestX = %d, obj->goblinY = %d, obj->gobDestY = %d)",
				obj->animName, obj->goblinX, obj->gobDestX, obj->goblinY, obj->gobDestY);

			animState = computeObjNextDirection(*obj);

			debug(SoftLevel::kInterpreter, "Obj %s new state = %d (obj->goblinX = %d, obj->gobDestX = %d, obj->goblinY = %d, obj->gobDestY = %d)",
				obj->animName, animState, obj->goblinX, obj->gobDestX, obj->goblinY, obj->gobDestY);
		}

		debug(SoftLevel::kInterpreter, "Obj % s initiateMove(lookDir = % d, obj->posX = % d, obj->posY = % d, obj->goblinX = % d, obj->gobDestX = % d, obj->goblinY = % d, obj->gobDestY = % d)",
