...
namespace Gob {
	...
	Inter::Inter(GobEngine* vm) : _varStack(1000), _vm(vm) {
		_terminate = 0;
		_break = false;
	...
	void Inter::executeOpcodeDraw(byte i) {
		debug(SoftLevel::kInterpreter, "opcodeDraw %d [0x%X] (%s)", i, i, getDescOpcodeDraw(i));
	...
	void Inter::executeOpcodeFunc(byte i, byte j, OpFuncParams& params) {
		debug(SoftLevel::kInterpreter, "%s:%08d: opcodeFunc %d.%d [0x%X.0x%X] (%s)",
	...
	void Inter::executeOpcodeGob(int i, OpGobParams& params) {
		debug(SoftLevel::kInterpreter, "opcodeGoblin %d [0x%X] (%s)",
