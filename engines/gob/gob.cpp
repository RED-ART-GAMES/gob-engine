...
#define MAX_TIME_DELTA 100
	...
	GobEngine::GobEngine(OSystem* syst, const bool wipeDataAtLaunch, const bool silenceAdlibInGob1Windows)
		: Engine(syst)
		, _rnd("gob")
		, _wipeDataAtLaunch(wipeDataAtLaunch)
	{
		_silenceAdlibInGob1Windows = silenceAdlibInGob1Windows;
		_sound = nullptr; 
		_mult = nullptr; 
		_game = nullptr;
		_global = nullptr; 
		...
		_mixer->setVolumeForSoundType(Audio::kSFXSoundType,
			muteSFX ? 0 : ConfMan.getInt("sfx_volume"));
		_mixer->setVolumeForSoundType(Audio::kMusicSoundType,
	...
	Common::Platform GobEngine::getTracksPlatform() const {
		return _tracksPlatform;
	}
	...
	bool GobEngine::totRequiresMayoHack(uint32_t& takeOffset, uint32_t& dropOffset) const {
		if (getGameType() != kGameTypeGob2 || _language == Common::EN_USA || isCD())
		{
			return false;
		}

		if (isCurrentTot("GOB10.TOT"))
		{
			takeOffset = 14713;
			dropOffset = 7037;
		}
		else if (isCurrentTot("GOB09.TOT"))
		{
			takeOffset = 13487;
			dropOffset = 6841;
		}
		else if (isCurrentTot("GOB11.TOT"))
		{
			takeOffset = 16468;
			dropOffset = 8883;
		}
	...
	int32 GobEngine::getFeatures() const { 
		return _features; 
	}
	...
	Common::Error GobEngine::run(const bool shouldSkipIntro, const int forcedToLevel) {
		Common::Error err;

		err = initGameParts(shouldSkipIntro);
		if (err.getCode() != Common::kNoError) {
			warning("initGameParts failed...");
			return err;
		}

		err = initGraphics();
		if (err.getCode() != Common::kNoError) {
			warning("initGraphics failed...");
			return err;
		}
		...
		if (isCD()) {
			if (!existExtractedCDAudioFiles()) {
				if (!isDataAndCDAudioReadFromSameCD())
...
#if defined(_DEBUG) || defined(_FOR_QA)
		if (NULL == _debug) _debug = new Gob::Debug();
		_debug->_levelToLoad = forcedToLevel;
#endif

		// We can relaunch the current game by returning `true`
		if (_init->initGame()) return Common::kLaunchGame;
	...
	Common::Error GobEngine::initGameParts(const bool shouldSkipIntro) {
		...
		_game = new Game(this, shouldSkipIntro);
