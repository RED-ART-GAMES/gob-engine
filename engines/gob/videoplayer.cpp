...
namespace Gob {
	...
	bool VideoPlayer::playFrame(int slot, Properties& properties) {
		...
		if (surface && primary) {
			...
			const Common::List<Common::Rectangle>& dirtyRects = video->decoder->getDirtyRects();
			...
			if (video->surface == _vm->_draw->_backSurface) {
				...
				for (Common::List<Common::Rectangle>::const_iterator rect = dirtyRects.begin(); rect != dirtyRects.end(); ++rect)
	...
	const Common::List<Common::Rectangle>* VideoPlayer::getDirtyRects(int slot) const {
		const Video* video = getVideoBySlot(slot);
		if (!video)
			return 0;
	...
	::Video::CoktelDecoder* VideoPlayer::openVideo(const Common::String& file, Properties& properties) {
		...
		// We shouldn't have RMD & PreIMD
		if (properties.type == kVideoTypeIMD)
			video = new ::Video::IMDDecoder(_vm->_mixer, Audio::kSFXSoundType);
		/*else if (properties.type == kVideoTypePreIMD)
			...
			video = new ::Video::VMDDecoder(_vm->_mixer, Audio::kSFXSoundType);
		else if (properties.type == kVideoTypeRMD)
			video = new ::Video::VMDDecoder(_vm->_mixer, Audio::kSFXSoundType);
		else
			error("Couldn't open video \"%s\": Invalid video Type", fileName.c_str());
