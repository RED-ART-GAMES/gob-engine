...
namespace Gob {
	...
	namespace Geisha {
		...
		Penetration::Penetration(GobEngine* vm) : _vm(vm), _background(nullptr), _sprites(nullptr), _objects(nullptr), 
			_shieldMeter(nullptr), _healthMeter(nullptr), _floor(0), _sub(nullptr), _isPlaying(false) {
		...
		void Penetration::updateAnims() {
			...
			for (Common::List<ANIObject*>::reverse_iterator a = _mapAnims.reverse_begin();
				a != _mapAnims.reverse_end(); ++a) {
			...
			for (Common::List<ANIObject*>::reverse_iterator a = _anims.reverse_begin();
				a != _anims.reverse_end(); ++a) {
